...
def get_bkgcat3_events_forApply(filename,mx, my):
    ...
    events = eventsSL
    return events
def get_ddcat3_events_forApply(filename,mx, my):
    events = ak.from_parquet(filename)
    category_cut = ((events["category"]==3) | (events["category"]==4))
    photonID_cut = (events["Diphoton_minID_modified"]>-0.7)
    events = events[ category_cut & photonID_cut]
    ...
    events = get_met_variables(events)
    eventsSL=events[events["category"]==3]
    eventsSL = get_FHSL_jet_W_variables(eventsSL, "FH")
    eventsSL = deal_with_none_object(eventsSL, "FH")
    events = eventsSL
    return events
def get_ddcat4_events_forApply(filename,mx, my):
    events = ak.from_parquet(filename)
    category_cut = ((events["category"]==3) | (events["category"]==4))
    photonID_cut = (events["Diphoton_minID_modified"]>-0.7)
    events = events[ category_cut & photonID_cut]
...
events_dd_cat3= get_ddcat3_events_forApply(bkgfiles[2],mx,my)
...
def process_sig_samples(FHfile,SLfile,bbggfile,zzggfile,sigoutput,bbggoutput,zzggoutput,model_path,input_features,cut1,cut2):
    ...
    def evaluate_model(events,model):
        ...
        PBDT_score = ( PBDT_score[:, 2] + PBDT_score[:, 3]) / (
