...
parser.add_argument('--model',type=str,default="/eos/user/z/zhenxuan/PNN_wwgg/PNN_YH_combined/boosted_FHSL/data/model_PNN_FHSL_boosted.h5", help='model file')
parser.add_argument('--scalar',type=str,default="/eos/user/z/zhenxuan/PNN_wwgg/PNN_YH_combined/boosted_FHSL/data/scaler_params.json", help='scalar file')
...
def target_function(bo1, bo2,s, b, bkg):
    ...
    # events_sig3 = s[(s['PNN_score'] >= 1-bo3-bo2-bo1) & (s['PNN_score'] <1-bo2-bo1)]
    # events_datasideband_3 = b[(b['PNN_score'] >= 1-bo3-bo2-bo1) & (b['PNN_score'] <1-bo2-bo1)]
    # events_sig4 = s[(s['PNN_score'] >= 1-bo4-bo3-bo2-bo1) & (s['PNN_score'] <1-bo3-bo2-bo1)]
    # events_datasideband_4 = b[(b['PNN_score'] >= 1-bo4-bo3-bo2-bo1) & (b['PNN_score'] <1-bo3-bo2-bo1)]
    # events_sig5 = s[(s['PNN_score'] >= 1-bo5-bo4-bo3-bo2-bo1) & (s['PNN_score'] <1-bo4-bo3-bo2-bo1)]
    # events_datasideband_5 = b[(b['PNN_score'] >= 1-bo5-bo4-bo3-bo2-bo1) & (b['PNN_score'] <1-bo4-bo3-bo2-bo1)]
    ...
    if len(events_datasideband_1[((events_datasideband_1.Diphoton_mass>135)|(events_datasideband_1.Diphoton_mass<115))]) < 5:
        ...
        significance=0
        return 0
    # elif len(events_datasideband_3[((events_datasideband_3.Diphoton_mass>135)|(events_datasideband_3.Diphoton_mass<115))]) < 10:
    #     significance=0                        
    #     return 0
    # elif len(events_datasideband_4[((events_datasideband_4.Diphoton_mass>135)|(events_datasideband_4.Diphoton_mass<115))]) < 10:
    #     significance=0                        
    #     return 0
    # elif len(events_datasideband_5[((events_datasideband_5.Diphoton_mass>135)|(events_datasideband_5.Diphoton_mass<115))]) < 10:
    #     significance=0                        
    #     return 0
    if debug:
        print("len(events_datasideband_1): ", len(events_datasideband_1))
        print("len(events_datasideband_2): ", len(events_datasideband_2))
        # print("len(events_datasideband_3): ", len(events_datasideband_3))
        # print("len(events_datasideband_4): ", len(events_datasideband_4))
    ...
    # significance = np.sqrt(significance1**2 + significance2**2 + significance3**2 + significance4**2+ significance5**2)
...
#print events_sig shape
PNN_score = model.predict(loaded_scaler.transform(df[input_features].values))
PNN_score = (PNN_score[:,1] + PNN_score[:,2] + PNN_score[:,3]) / (PNN_score[:,0] + PNN_score[:,1] + PNN_score[:,2] + PNN_score[:,3])
...
# fig.show()
# plt.figure()
# plt.hist(event_bkgmc['PNN_score'],weights = event_bkgmc.weight_central ,bins=50, histtype='step', label='bkg',color='blue')
# plt.hist(events_sig['PNN_score'],weights = 30*events_sig.weight_central ,bins=50, histtype='step', label='30*signal',color='red')
# plt.hist(events_data['PNN_score'],weights = events_data.weight_central ,bins=50, marker="o", label='data',color='black')
# fig.savefig("/afs/cern.ch/user/s/shsong/WWggDNN/bdt/dnnscore.png", dpi=140)
...
def calculate_significance(events_sig, events_bkg, threshold0, threshold1,events_data):
    ...
    df_sig = pd.DataFrame()
    df_sig['sig_PNN_score'] = np.array(events_sig['PNN_score'])
    df_sig['sig_weight'] = np.array(events_sig['weight_central'])
    df_bkg = pd.DataFrame()
    df_bkg['bkg_PNN_score'] = np.array(events_bkg['PNN_score'])
    df_bkg['bkg_weight'] = np.array(events_bkg['weight_central'])
    df_data = pd.DataFrame()
    df_data['data_PNN_score'] = np.array(events_data['PNN_score'])
    df_data['data_weight'] = np.array(events_data['weight_central'])
    # 计算信号和背景的PNN score的cut
    signal_PNN_score_cut = (df_sig['sig_PNN_score'] > cut0) & (df_sig['sig_PNN_score'] <= cut1)
    background_PNN_score_cut = (df_bkg['bkg_PNN_score'] > cut0) & (df_bkg['bkg_PNN_score'] <= cut1)
    data_PNN_score_cut = (df_data['data_PNN_score'] > cut0) & (df_data['data_PNN_score'] <= cut1)
    # 计算信号和背景的PNN score的cut后的weight
    signal_weight = df_sig['sig_weight'][signal_PNN_score_cut]
    background_weight = df_bkg['bkg_weight'][background_PNN_score_cut]
    data_weight = df_data['data_weight'][data_PNN_score_cut]
...
r2=1-boundaries['bo1']-boundaries['bo2']
cut2=1-boundaries['bo1']-boundaries['bo2']

# add max_PNN_score to the dataframe with index
# max_PNN_score = 0
...
#     boundary={str(Xmass):{"cut1":cut1,"cut2":cut2,"FHSL_highpurity":FHSL_highpurity,"FHSL_lowpurity":FHSL_lowpurity,"BBGG_highpurity":bbgg_highpurity,"BBGG_lowpurity":bbgg_lowpurity,"ZZGG_highpurity":zzgg_highpurity,"ZZGG_lowpurity":zzgg_lowpurity}}
