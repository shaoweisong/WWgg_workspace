...
def process_sig_samples_wrapper(args):
    FHfile, SLfile, bbggfile, zzggfile, sig_output_name, bbgg_output_name, zzgg_output_name, model_path, input_features, cut1, cut2 = args
    result = process_sig_samples(FHfile, SLfile, bbggfile, zzggfile, sig_output_name, bbgg_output_name, zzgg_output_name, model_path, input_features, cut1, cut2)
    return result
args_list = [
    (signal_samples['FHpath'][i], signal_samples['SLpath'][i], signal_samples['BBGGpath'][i], signal_samples['ZZggpath'][i],
     signal_samples['sig_output_name'][i], signal_samples['bbgg_output_name'][i], signal_samples['zzgg_output_name'][i],
     model_path, input_features, cut1, cut2) 
    for i in range(1, len(signal_samples['FHpath']))
]
pool = multiprocessing.Pool(processes=8)
with tqdm(total=len(args_list)) as pbar:
    for result in pool.imap_unordered(process_sig_samples_wrapper, args_list):
