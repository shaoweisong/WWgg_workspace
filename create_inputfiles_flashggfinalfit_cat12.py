...
for i in range(len(signal_samples['FHpath'])):
    mx = signal_samples['sig_output_name'][i].split('_')[1].split('X')[1]
    my = signal_samples['sig_output_name'][i].split('_')[2].split('H')[1]
    events_sigFH = get_sig_events_forApply(signal_samples['FHpath'][i],mx,my)
    events_sigSL = get_sig_events_forApply(signal_samples['SLpath'][i],mx,my)
    events_bbgg = get_sig_events_forApply(signal_samples['BBGGpath'][i],mx,my)
    events_zzgg = get_sig_events_forApply(signal_samples['ZZggpath'][i],mx,my)
    ...
    events_dd_cat1= get_data_events_forApply(bkgfiles[2],mx,my)
    events_dd_cat2= get_data_events_forApply(bkgfiles[3],mx,my)
    ...
    print('get all the PBDT score')
    
    #-------------------------------------Bayesian Optimization-------------------------------------#
    def calculate_significance(events_sig, events_bkg, threshold0, threshold1):
        ...
        df_sig['Diphoton_mass'] = np.array(events_sig['Diphoton_mass'])
        df_sig['sig_weight'] = np.array(events_sig['weight_central'])
        
        df_bkg = pd.DataFrame()
        df_bkg['bkg_PBDT_score'] = np.array(events_bkg['PBDT_score'])
        df_bkg['Diphoton_mass'] = np.array(events_bkg['Diphoton_mass'])
        df_bkg['bkg_weight'] = np.array(events_bkg['weight_central'])
        signal_mass= (df_sig['Diphoton_mass']>115) & (df_sig['Diphoton_mass']<135)
        bkg_mass= (df_bkg['Diphoton_mass']>115) & (df_bkg['Diphoton_mass']<135)
        df_sig=df_sig[signal_mass]
        df_bkg=df_bkg[bkg_mass]
        # 计算信号和背景的PNN score的cut
        signal_pnn_score_cut = (df_sig['sig_PBDT_score'] > cut0) & (df_sig['sig_PBDT_score'] <= cut1)
        background_pnn_score_cut = (df_bkg['bkg_PBDT_score'] > cut0) & (df_bkg['bkg_PBDT_score'] <= cut1)
    ...
    def objective(thresholds):
            ...
            significance1 = calculate_significance(events_sig, event_bkgmc, 0, threshold1)
            significance2 = calculate_significance(events_sig, event_bkgmc, threshold1, threshold2)
            return -(np.sqrt(significance1) + np.sqrt(significance2))

        # 贝叶斯优化

    if "merged_nominal" in signal_samples['sig_output_name'][i]:
        print('start to do Bayesian Optimization')
        bounds = [(0.001, 0.6), (0.2, 0.4)]
        initial_guess = [0.001, 0.3]
        options = {'maxiter': 200}
        result = minimize(objective, initial_guess, method='L-BFGS-B', bounds=bounds, options=options)
    ...
    def add_shape_uncertainty_br(events):
        ...
        return events
    sig_components = [events_sig, events_bbgg, events_zzgg]
    if "merged_nominal" in signal_samples['sig_output_name'][i]:
        for sig in sig_components:
            sig = add_sf_branches(sig)
    else:
        for sig in sig_components:
            sig = add_shape_uncertainty_br(sig)
    ...
    ak.to_parquet(events_sig_highpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['sig_output_name'][i]+"_highpurity.parquet")
    ak.to_parquet(events_sig_lowpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['sig_output_name'][i]+"_lowpurity.parquet")
    ...
    ak.to_parquet(events_bbgg_highpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['bbgg_output_name'][i]+"_highpurity.parquet")
    ak.to_parquet(events_bbgg_lowpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['bbgg_output_name'][i]+"_lowpurity.parquet")
    ...
    ak.to_parquet(events_zzgg_highpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['zzgg_output_name'][i]+"_highpurity.parquet")
    ak.to_parquet(events_zzgg_lowpurity,"./PBDT_HH_FHSL_combine_2017/"+signal_samples['zzgg_output_name'][i]+"_lowpurity.parquet")
